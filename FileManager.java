...
public class FileManager {
        ...
        Function<String, Task> createTask =
                record -> {
                            ...
                            if(parts.length == TaskFields.NUMBER_OF_FIELDS.toInt()) {
                                try {
                                    String title = parts[TaskFields.TITLE.toInt()].trim().replace("title = ", "");
                                    String project = parts[TaskFields.PROJECT.toInt()].trim().replace("project = ", "");
                                    LocalDate dueDate = Utility.convertDate(parts[TaskFields.DUE_DATE.toInt()].trim().replace("date = ",""));
                                    boolean status = parts[TaskFields.STATUS.toInt()].contains("In progress") ? false : true;
     ...
     */
    private static String handleSpecialCharacters(String data) {
